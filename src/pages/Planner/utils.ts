...
export type BaseTag = "focus" | "meeting" | "errand";
...
export type JalaliMonthDays = Array<number | null>;
...
export type PlannerState = {
  ...
  now: Date;
  jalaliMonthView: JalaliDateParts;
  tagModalOpen: boolean;
  tagModalValue: string;
  editingTag: string | null;
  tagModalError: string;
  formError: string;
  priorities: Priority[];
  newTaskPriority?: string;
  undoToast: UndoPayload | null;
  undoTimer: number | null;
  toastKey: number;
  deleteModal: DeleteModalState | null;
  deleteModalBusy: boolean;
  poolHover: boolean;
  calendarModal: null | "month" | "year";
  copiedTaskId: string | null;
  copiedTask: CopiedTask | null;
};

type PlannerAction =
  | { type: "set"; payload: Partial<PlannerState> }
  | { type: "update"; updater: (state: PlannerState) => PlannerState };
...
export const tagLabels: Record<BaseTag, string> = {
  focus: "تمرکز",
  meeting: "جلسه",
  errand: "کارهای ریز",
};

export const baseTags: BaseTag[] = ["focus", "meeting", "errand"];
...
export function createPlannerInitialState(): PlannerState {
...
export function priorityRank(id?: string) {
  if (!id) return 3;
  return PRIORITY_RANK[id] ?? 3;
}

export function sortByHour(list: ScheduledTask[]) {
...
export function mergeConsecutive(list: ScheduledTask[]): MergedBlock[] {
...
export function dedupe(list: ScheduledTask[]) {
...
export function getPriorityColor(id: string | undefined, priorities: Priority[]) {
...
export function dateKeyFromGregorian(gy: number, gm: number, gd: number) {
  return new Date(Date.UTC(gy, gm - 1, gd)).toISOString().slice(0, 10);
}

export function dateKeyFromJalali(jy: number, jm: number, jd: number) {
...
export function formatJalaliMonthName(date: JalaliDateParts) {
...
export function formatGregorianSpanForJalaliMonth(jy: number, jm: number) {
...
export function buildJalaliMonthDays(jy: number, jm: number): JalaliMonthDays {
...
export function buildYearOptions(current: number) {
...
export function isGregorianLeap(year: number) {
